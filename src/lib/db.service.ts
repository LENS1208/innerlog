...
export async function getAllTrades(dataset?: string): Promise<DbTrade[]> {
  ...
  console.log(`✅ Loaded from database: ${allTrades.length} trades (dataset: ${dataset || 'all'})`);
  return allTrades;
}
...
export async function insertTrades(trades: Omit<DbTrade, 'id' | 'created_at' | 'user_id'>[], dataset?: string): Promise<void> {
  const { data: { user } } = await supabase.auth.getUser();

  const tradesWithUser = trades.map(trade => ({
    ...trade,
    user_id: user?.id || null,
    dataset: dataset || (trade as any).dataset || null,
  }));
