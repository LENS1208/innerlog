    ...
    <>
      <div className="kpi-card">
        ...
        <div className="kpi-value" style={{ color: getAccentColor() }}>
          +{summaryData.total_deposits.toLocaleString('ja-JP')} <span style={{ fontSize: '12px', color: getAccentColor() }}>円</span>
        </div>
      ...
      <div className="kpi-card">
        ...
        <div className="kpi-value" style={{ color: getLossColor() }}>
          -{summaryData.total_withdrawals.toLocaleString('ja-JP')} <span style={{ fontSize: '12px', color: getLossColor() }}>円</span>
        </div>
      ...
      {peakEquity !== undefined && (
        <div className="kpi-card">
          ...
          <div className="kpi-value" style={{ color: getAccentColor() }}>
            +{peakEquity.toLocaleString('ja-JP')} <span style={{ fontSize: '12px', color: getAccentColor() }}>円</span>
          </div>
      ...
      <div className="kpi-card">
        ...
        <div className="kpi-value" style={{ color: summaryData.total_swap >= 0 ? getAccentColor() : getLossColor() }}>
          {summaryData.total_swap >= 0 ? '+' : ''}{summaryData.total_swap.toLocaleString('ja-JP')} <span style={{ fontSize: '12px', color: summaryData.total_swap >= 0 ? getAccentColor() : getLossColor() }}>円</span>
        </div>
        {hasSwapBreakdown ? (
          <div className="kpi-desc" style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>
            <span style={{ color: getAccentColor() }}>+{(summaryData.swap_positive || 0).toLocaleString('ja-JP')}<span style={{ color: getAccentColor() }}>円</span></span>
            {' / '}
            <span style={{ color: getLossColor() }}>-{(summaryData.swap_negative || 0).toLocaleString('ja-JP')}<span style={{ color: getLossColor() }}>円</span></span>
          </div>
        ) : (
      ...
      {hasXmPoints && (
        <>
          <div className="kpi-card">
            ...
            <div className="kpi-value" style={{ color: getAccentColor() }}>
              +{summaryData.xm_points_earned.toLocaleString('ja-JP')} <span style={{ fontSize: '12px', color: getAccentColor() }}>円</span>
            </div>
